...
class OpenAgentsMarchantRecords 
{
    ...
    createUi()
    {
        if(this.#_mErrors == 0)
        {
            ...
            <div class="container">
                <div class="row">
                    <div class="col s12" style="margin-top: 4rem;">
                        <b class="purple-text">Marchant Customers</b>
                    </div>
                    <div class="col s12" style="margin-top: 2rem;">
                        This is a comprehensive list of all customers currently registered as marchant-user by agents.
                    </div>
                    <div class="col s12" style="margin-top: 2rem;">
            ...
            if(document.querySelector('.dashboard') && sessionStorage.getItem('agentId'))
            {
                document.querySelector('.dashboard').addEventListener('click', e => {
                    e.preventDefault();
                    import('./agent_customer_profile_dashboard.js')
                    .then(m => {
                        let userId = sessionStorage.getItem('agentId');
                        let dashboard = new m.AgentProfileDashboard(userId);
                        dashboard.createUi();
                    })
                    .catch(error => {
                        console.log(error.message);
                        toastIt('red', 'Unable to load agent dashborad');
                    });
                });
            }
...
export { OpenAgentsMarchantRecords };